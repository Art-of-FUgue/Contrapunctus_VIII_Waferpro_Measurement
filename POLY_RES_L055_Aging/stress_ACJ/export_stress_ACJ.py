...
def genMEA1(write_tmp, setup_name, x_name, y1_name,y2_name,y3_name, y4_name, y5_name):
    import iccap
#    print(0)
...
#    print(4)
    y4_data = iccap.get_dataset(setup_name +'/'+ y4_name, True)[('M','11')]
#    print(5)
    y5_data = iccap.get_dataset(setup_name +'/'+ y5_name, True)[('M','11')]
    for x, y1,y2,y3,y4,y5 in zip(x_data, y1_data, y2_data, y3_data, y4_data, y5_data):
            write_tmp += '%s\t%s\t%s\t%s\t%s\t%s\n' %(x.real, y1.real, y2.real, y3.real, y4.real, y5.real)
    return write_tmp
...
stresstime = float(get_var('../runPolyaging_ACJ/Tot_Stress_Time'))
...
write_tmp += 'Page (name={}'.format(indicator)+ 'x=time, y1=iA, y2=iC, y3=vA, y4=vJ,y5=vC) {T=%s}\n' %(temp)
setup_name = dut_name + '/stress_ACJ'
write_tmp = genMEA1(write_tmp, setup_name,'time', 'iA', 'iC', 'vA', 'vJ','vC')

filePath = get_var(dut_name+'/Mea_Data_Save_Dir')+'\\'+Lot_str+'\\'+Wafer_str+'\\'+str(temp)+'c\\'+die_index+'\\'+Device_Dir+'\\'+'stress\\'
instanceInfo = indicator
fileName = get_var('../runPolyaging_ACJ/filename')+'stress_time={}'.format(stresstime)+'.mea'
